...
export default function SEOAnalyzer() {
  ...
  const analyzeSEO = async () => {
    ...
    try {
      const API_URL = process.env.NEXT_PUBLIC_FASTAPI_URL || 'http://localhost:5000';
      const response = await fetch(`http://localhost:5000/api/crawl?url=${encodeURIComponent(url)}`);
